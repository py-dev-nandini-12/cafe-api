...
def get_random_cafe():
    ...
    return jsonify(random_cafes.to_dict())
...
def search_cafe_location():
    cafe_api_location = request.args.get("loc")

    result = db.session.execute(db.select(Cafe).where(Cafe.location==cafe_api_location))
    all_cafes =result.scalars().all()
    if all_cafes:
        return jsonify(cafes=[cafe.to_dict() for cafe in all_cafes ])
    else:
...
def update_coffe_price(cafe_id):
    ...
    if cafe:
        ...
        return jsonify(error={"Not found": "Cafe with the id is not found in the database"}),404
